...
async def api_generate(request: Request):
    """
    服务端代理端点：前端调用此接口，后端使用环境变量中的 Token 调用 Agent API。
    解决跨域（CORS）问题，Token 不暴露给浏览器。
    """
    import os
    import httpx
    ...
    # 从环境变量获取 Token 和 API 地址
    api_token = os.getenv("COZE_LOOP_API_TOKEN", "")
    api_url = os.getenv("COZE_PROJECT_DOMAIN_DEFAULT", "https://74q7m3vpzh.coze.site")
    # 使用用户提供的部署地址（如果有）
    deploy_url = body.get("apiUrl", api_url)
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            async with client.stream(
                ...
                if response.status_code != 200:
                    ...
                    yield f"event: error\ndata: {json.dumps({'error': f'Agent API returned {response.status_code}: {error_body.decode()}'})}\n\n"
                    return
...
async def serve_frontend():
    """Serve the AI Creator Finder frontend page."""
